...
function SavedMovies({ handleIsLogin, isLoggedIn, saveMovieId, onDelete }) {
  ...
  function handleSearch(query) {
    const dataMovies = searchMovies(JSON.parse(localStorage.getItem('saveMovies')), query);
  ...
  function handleCheckbox(event) {
    ...
    setIsShort(checkbox);
    const dataMovies = searchMovies(JSON.parse(localStorage.getItem('saveMovies')), isQuery);
  ...
  return (
    <>
      ...
      <main className="saved-movies">
        <SearchForm onSearch={handleSearch} isQuery={isQuery} setIsQuery={setIsQuery} onCheckbox={handleCheckbox} isShort={isShort} />
        <MoviesCardList
          movies={isSaveMovies}
