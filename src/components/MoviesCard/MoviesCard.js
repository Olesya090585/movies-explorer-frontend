...
function MoviesCard({ card, handleSaveMovie, onDelete, saveMovieId, isSaveMovies, setIsSaveMovies }) {
  ...
  const classButton = `movie__button ${isSaved ? 'movie__button_active' : ''} ${path ? 'movie__button-delete' : ''}`;

  function handleSaveClick() {
    if (path) {
      onDelete(card);
      setIsSaveMovies((state) => state.filter((c) => c._id !== card._id));
    } else {
      if (isSaved) {
